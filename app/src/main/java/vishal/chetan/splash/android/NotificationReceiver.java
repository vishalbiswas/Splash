...
public class NotificationReceiver extends BroadcastReceiver {
    ...
    private void checkNotifications(int serverIndex) {
        if (GlobalFunctions.servers.get(serverIndex).identity != null) {
            ...
            if (notification_indices.get(serverIndex) == null) {
                notification_indices.append(serverIndex, new LongSparseArray<Integer>());
                path += "all";
            } else {
                path += "unread";
            }
            new AsyncArrayHelper(serverIndex, path, "sessionid=" + GlobalFunctions.servers.get(serverIndex).identity.getSessionid()) {
                @Override
                            ...
                            try {
                                ...
                                if (!notification.isDone()) {
                                    if (notification.actionuid > 0) {
                                        ...
                                        showNotification(notifications.size() - 1, null);
                                    }
                                }
                                if (listener != null) {
    ...
    private void getMessageAndIntent(final SplashNotification notification) {
        SplashCache.UsersCache.getUser(notification.serverIndex, notification.actionuid, new SplashCache.UsersCache.OnGetUserListener() {
            ...
            public void onGetUser(final UserIdentity user) {
                if (user != null) {
                    final Thread thread = SplashCache.ThreadCache.getThread(notification.serverIndex, notification.threadid);
    ...
    private void showCommentNotification(final SplashNotification notification) {
        final Thread thread = SplashCache.ThreadCache.getThread(notification.serverIndex, notification.threadid);
        SplashCache.AttachmentCache.get(notification.serverIndex, SplashCache.UsersCache.getUser(notification.serverIndex, notification.actionuid, null).getProfpic(), new SplashCache.AttachmentCache.OnGetAttachmentListener() {
            @Override
                ...
                thread.getCommentAsync(notification.commentid, new Thread.LoadCommentsListener() {
                    ...
                    public void onCommentsLoaded(boolean result) {
                        if (result && thread.getComment(notification.commentid) != null) {
                            ...
                            showNotification(notifications.size() - 1, getThumbnail(attachment));
                        }
                    }
                });
            }
